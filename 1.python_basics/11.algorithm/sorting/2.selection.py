# 특징: 매번 최솟값(또는 최댓값)을 "선택"해서 앞으로 보냄
# 시간복잡도: 항상 O(n²)
...
def selection_sort_with_steps(arr):
    ...
    for i in range(n - 1):
        ...
        if min_idx != i:
            arr[i], arr[min_idx] = arr[min_idx], arr[i]
            print(f"Step {i + 1}: {arr} (최솟값 {arr[i]}을 위치 {i}로 이동)")
        else:
            print(f"Step {i + 1}: {arr} (이미 올바른 위치)")
    
    return arr

def find_max_selection_sort(arr):
    """
    최댓값을 찾아서 뒤로 보내는 선택 정렬 변형
    """
    n = len(arr)
