...
class Music(db.Model):
    ...
    comments = db.relationship('Comment', backref='music', lazy=True, cascade='all, delete-orphan')
    likes = db.relationship('Like', backref='music', lazy=True, cascade='all, delete-orphan')
    notifications = db.relationship('Notification', backref='music', lazy=True, cascade='all, delete-orphan')
    hashtags = db.relationship('MusicHashtag', backref='music', lazy=True, cascade='all, delete-orphan')
...
class Notification(db.Model):
    ...
    def __repr__(self):
        return f'<Notification(notification_id={self.notification_id}, user_id={self.user_id}, music_id={self.music_id}, comment_id={self.comment_id}, message="{self.message}", is_read={self.is_read}, created_at={self.created_at})>'

class Hashtag(db.Model):
    hashtag_id = db.Column(db.Integer, primary_key=True)
    tag = db.Column(db.String(120), unique=True, nullable=False)

    musics = db.relationship('MusicHashtag', backref='hashtag', lazy=True, cascade='all, delete-orphan')
