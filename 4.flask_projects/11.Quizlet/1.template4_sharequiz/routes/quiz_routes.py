...
@quiz_bp.route('/share/<int:file_id>', methods=['POST'])
@login_required
def toggle_share(file_id):
    ...
    conn.commit()
    conn.close()
    flash('공개 설정이 업데이트되었습니다.' if is_public else '비공개로 전환되었습니다.', 'success')
    return redirect(url_for('quiz.dashboard'))

@quiz_bp.route('/search')
@login_required
def search():
    """공유된 공개 퀴즈 검색"""
    ...
    if q:
        rows = conn.execute('''
            ...
            WHERE qf.is_public = 1
              AND (qf.original_filename LIKE ? OR qf.filename LIKE ?)
        ...
        ''').fetchall()
    conn.close()
    return render_template('quiz/search.html', items=rows, q=q)

@quiz_bp.route('/import/<int:file_id>', methods=['POST'])
@login_required
def import_shared(file_id):
    """공개 퀴즈를 내 계정으로 가져오기"""
    import shutil
    conn = get_db_connection()
    row = conn.execute('''
    ...
    if not os.path.exists(src_path):
        conn.close()
        flash('원본 파일이 존재하지 않습니다.', 'error')
        return redirect(url_for('quiz.search'))

    user_folder = create_user_upload_folder(current_user.id)
    base_name, ext = os.path.splitext(row['original_filename'])
    safe_name = secure_filename(f"{base_name}{ext}")
    dst_path = os.path.join(user_folder, safe_name)
    counter = 1
    while os.path.exists(dst_path):
        safe_name = secure_filename(f"{base_name}_imported_{counter}{ext}")
        dst_path = os.path.join(user_folder, safe_name)
        counter += 1
    ...
    flash('내 퀴즈로 가져왔습니다. 대시보드에서 확인하세요.', 'success')
    return redirect(url_for('quiz.dashboard'))
